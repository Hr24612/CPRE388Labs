...
public class FlashModel implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_LIGHT) {
            ...
            if(event.values[0] >= 35 && event.values[0] <= 80 && messageHasStarted){
                mCallback.onColorSensed("RED");
                messageHasStarted = true;
                prevLightValue = RED;
            }
            if(event.values[0] >= 330 && event.values[0] <=350 &&messageHasStarted == true){
                mCallback.onColorSensed("WHITE");
                prevLightValue = WHITE;
            }
            if(event.values[0] >=0 && event.values[0] <= 30 && messageHasStarted == true){
                mCallback.onColorSensed("BLACK");
                prevLightValue = BLACK;
            }
            //TODO - if its red, then use the callback with update("RED") and set the messageHasStarted accordingly
            //TODO - if its white, then use the callback with update("WHITE")
            //TODO - if its black, then use the callback with update("BLACK")
            //TODO - don't forget to use and then also set the prevLightValue accordingly
        }
    }
    ...
    public ArrayList<Integer> encodeMessage(String message) {
        //TODO - encode the input string to an arrayList of DOTs,DASHes, and END_CHARACTERs to be matched up with colors, the first character encoding is done for you
        ...
        for (int i = 0; i < message.length(); i++) {//for each character in the input string, append its encoding to the encodedMessage arrayList
            if (message.charAt(i) == 'a') {
                ...
                encodedMessage.addAll(Arrays.asList(DOT, DOT, DASH, DOT, DOT, END_CHARACTER));
            }
                ...
                encodedMessage.addAll(Arrays.asList(DOT, DOT, DASH, END_CHARACTER));
            }
            else if(message.charAt(i) == 'v'){
                encodedMessage.addAll(Arrays.asList(DOT, DOT, DOT, DASH, END_CHARACTER));
            }
            else if(message.charAt(i) == 'w'){
                encodedMessage.addAll(Arrays.asList(DOT, DASH, DASH, END_CHARACTER));
            }
            else if(message.charAt(i) == 'x'){
                encodedMessage.addAll(Arrays.asList(DASH, DOT, DOT, DASH, END_CHARACTER));
            }
            else if(message.charAt(i) == 'y'){
                encodedMessage.addAll(Arrays.asList(DASH, DOT, DASH, DASH, END_CHARACTER));
            }
            else if(message.charAt(i) == 'z'){
                encodedMessage.addAll(Arrays.asList(DASH, DASH, DOT, DOT, END_CHARACTER));
            }
    ...
    public char decodeCharacterPattern(ArrayList<String> patternList) {
        //TODO - decode the arrayList of patterns seen and output the corresponding character that the pattern represents, the first character is done for you
        ...
        else if (patternList.equals(Arrays.asList(DOT, DOT, DASH, DOT, DOT, END_CHARACTER))){
            return 'h';
        }
        ...
        else if (patternList.equals(Arrays.asList(DOT, DOT, DASH, END_CHARACTER))){
            return 'u';
        }
        else if (patternList.equals(Arrays.asList(DOT, DOT, DOT, DASH, END_CHARACTER))){
            return 'v';
        }
        else if (patternList.equals(Arrays.asList(DOT, DASH, DASH, END_CHARACTER))){
            return 'w';
        }
        else if (patternList.equals(Arrays.asList(DASH, DOT, DOT, DASH, END_CHARACTER))){
            return 'x';
        }
        else if (patternList.equals(Arrays.asList(DASH, DOT, DASH, DASH, END_CHARACTER))){
            return 'y';
        }
        else if (patternList.equals(Arrays.asList(DASH, DASH, DOT, DOT, END_CHARACTER))){
            return 'z';
        }
