...
public class FlashActivity extends AppCompatActivity implements SensorUpdateCallback {
    ...
    public void sendWithFlashes(View view) {
        ...
        if (!messageToSend.equals("")) {//if the user has entered a message, then go ahead
            ...
            //TODO - encode the message and store it in a already defined global variable
           encodedMessage= flashModel.encodeMessage(messageToSend);

            //TODO - start the handler
            startTimer.run();
        }
    }
    ...
    @Override
    public void onColorSensed(String value) {

        //TODO - Finish this method's implementation, Make sure you think about what exactly is suppose to happen in here and how you can make sure it happens
        ...
        if (!endOfMessage) {//if the end of the message hasn't yet been detected, then see what color has been sensed from the light sensor
            //TODO - if value = "RED", initialize the messageReceived arrayList and clear the messageReceivedTextView
            if(value.equals("RED")){
                ArrayList<String> messageReceived = new ArrayList<>();
                messageReceived.add("");
                messageReceivedTextView.setText("");
            }
            //TODO - if value = "BLACK", get the current time and store it into rStartTime
            if(value.equals("BLACK")){
                rStartTime = System.currentTimeMillis();
            }
            //TODO - if value = "WHITE", find the total time that the screen was black, and add FlashModel.DOT, .DASH, or .END_CHARACTER accordingly to messageReceived arrayList
            if(value.equals("WHITE")){
                long time = System.currentTimeMillis() - rStartTime;
                if(time >= 150 && time <= 450){
            ...
            //TODO - when need be, decode the pattern and update the messageReceivedTextView
        }
    }
    ...
    private void changeScreenColor() {
        //TODO - iterate through the encodedMessage arrayList using messageCounter as the index.
        ...
        if (messageCounter < encodedMessage.size()) {
